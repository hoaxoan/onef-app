...
import 'package:onef/widgets/nav_bars/curves.dart';
import 'package:onef/widgets/nav_bars/fluid_button.dart';
import 'package:onef/widgets/nav_bars/fluid_icon_data.dart';

typedef void FluidNavBarChangeCallback(int selectedIndex);

class OFFluidNavBar extends StatefulWidget {
  static const double nominalHeight = 56.0;

  final FluidNavBarChangeCallback onChange;

  OFFluidNavBar({this.onChange});

  @override
  State createState() => _OFFluidNavBarState();
}

class _OFFluidNavBarState extends State<OFFluidNavBar>
  ...
  Widget build(context) {
    ...
    final height = OFFluidNavBar.nominalHeight;
    return Container(
      width: appSize.width,
      height: OFFluidNavBar.nominalHeight,
      child: Stack(
        children: [
          Positioned(
            left: 0,
  ...
  List<FluidNavBarButton> _buildButtons() {
    ...
    int buttonCount = 4;
    var buttons = List<FluidNavBarButton>(buttonCount);
