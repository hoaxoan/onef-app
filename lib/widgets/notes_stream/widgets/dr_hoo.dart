...
class OFNotesStreamDrHoo extends StatelessWidget {
  final VoidCallback streamRefresher;
  final OFNotesStreamStatus streamStatus;
  final List<Widget> streamPrependedItems;

  const OFNotesStreamDrHoo({
    Key key,
  ...
  Widget build(BuildContext context) {
    ...
    switch (streamStatus) {
      case OFNotesStreamStatus.refreshing:
    ...
    if (hasRefreshButton) {
      drHooColumnItems.addAll([
        ...
        OFButton(
          ...
          isLoading: streamStatus == OFNotesStreamStatus.refreshing,
        )
      ]);
    }
      ...
      padding: EdgeInsets.symmetric(
          ...
          vertical:
              streamPrependedItems != null && streamPrependedItems.isNotEmpty ||
                      streamStatus == OFNotesStreamStatus.empty ||
                      streamStatus == OFNotesStreamStatus.refreshing ||
                      streamStatus == OFNotesStreamStatus.loadingMoreFailed
                  ? 20
                  : 0),
